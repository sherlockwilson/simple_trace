...
SimpleTCP::CreateContext(uint64_t role_type) {
    if((role_type & RoleType::Client)|| (role_type & RoleType::Proxy)) {
        send_fd_ = socket(PF_INET, SOCK_STREAM, 0);
    }
    if((role_type & RoleType::Server) || (role_type & RoleType::Proxy)) {
...
SimpleTCP::ReqPacket(
    ...
    if(write(socket_fd, (char*)(req_packet.data_.c_str()), req_packet.data_.length()) == -1) {
        close(socket_fd);
        return false;
    }
    rsp_packet.data_.resize(recv_buffer_len_);
    if(read(socket_fd, (char*)(rsp_packet.data_.c_str()), rsp_packet.data_.length()) != -1) { 
        close(socket_fd);
        return false;
    }
    close(socket_fd);
...
bool
SimpleTCP::WaitProxyReqPacket(
        const std::string& des_ip,
        uint16_t des_port,
        HandlePacket client_req_handler,
        HandlePacket server_rsp_handler) {
    struct sockaddr_in client_addr;
    socklen_t sock_len;
    int32_t peer_fd = accept(recv_fd_, (struct sockaddr*)&client_addr, &sock_len);
    if(peer_fd < 0)  {
    ...
    client_req_handler(recv_packet);
    recv_packet.peer_ip_ = des_ip;
    recv_packet.peer_port_ = des_port;
    Packet rsp_packet;
    if(!ProxyPacket(recv_packet, rsp_packet)) {
        close(peer_fd);
        return false;
    }
    server_rsp_handler(rsp_packet);
    if(write(peer_fd, (char*)(rsp_packet.data_.c_str()), rsp_packet.data_.length()) == -1) {
       close(peer_fd);
       return false;
    }
    close(peer_fd);
